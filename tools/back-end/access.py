...
chat_platform = "zhipuai"

df_access = [("@gYaKlfLGewYYkmZb7T3BlbkFJouvA5wegTYHbPcgWgI4D", "sk-1Ni")]


HXQ_LLM_TOKEN = os.environ.get(
    "HXQ_LLM_TOKEN",
    "",
)
...
df_eet = {
    ...
    "english": {
        "Justice:Appeal": ["Defendant", "Adjudicator", "Crime", "Time", "Place"],
        "Justice:Extradite": [
            "Agent",
            "Person",
            "Destination",
        ...
        "Justice:Acquit": ["Defendant", "Adjudicator", "Crime", "Time", "Place"],
        "Life:Be-Born": ["Person", "Time", "Place"],
        "Life:Divorce": ["Person", "Time", "Place"],
        "Personnel:Nominate": ["Person", "Agent", "Position", "Time", "Place"],
        ...
        "Business:Start-Org": ["Agent", "Org", "Time", "Place"],
        "Business:End-Org": ["Org", "Time", "Place"],
        "Life:Injure": ["Agent", "Victim", "Instrument", "Time", "Place"],
        "Justice:Fine": ["Entity", "Adjudicator", "Money", "Crime", "Time", "Place"],
        "Justice:Sentence": [
            "Defendant",
            "Adjudicator",
            "Crime",
        ...
        "Justice:Execute": ["Person", "Agent", "Crime", "Time", "Place"],
        "Justice:Sue": [
            "Plaintiff",
            "Defendant",
            "Adjudicator",
        ...
        "Personnel:Elect": ["Person", "Entity", "Position", "Time", "Place"],
        "Business:Declare-Bankruptcy": ["Org", "Time", "Place"],
        ...
        "Justice:Release-Parole": ["Person", "Entity", "Crime", "Time", "Place"],
        "Conflict:Demonstrate": ["Entity", "Time", "Place"],
        "Contact:Phone-Write": ["Entity", "Time"],
        "Justice:Convict": ["Defendant", "Adjudicator", "Crime", "Time", "Place"],
...
def chatbot(mess):
    print(f"HXQ_LLM_TOKEN: {HXQ_LLM_TOKEN}, HXQ_LLM_TOKEN type: {type(HXQ_LLM_TOKEN)}")
    print(f"ZHIPU_API_KEY: {ZHIPU_API_KEY}, ZHIPU_API_KEY type: {type(ZHIPU_API_KEY)}")
    if chat_platform == "hxq":
        url = "https://llm.haoxinqing.cn/v1/chat/completions"

        payload = json.dumps({"model": "glm-4-9b-chat-int4", "messages": mess})
        headers = {
            "Authorization": f"Bearer {HXQ_LLM_TOKEN}",
        ...
        print(f"chatbot res_json: {res_json}")
        res = res_json["choices"][0]["message"]["content"]
        print(f"res: {res}")
    elif chat_platform == "zhipuai":
...
def chatie(input_data):
    ...
    if task == "RE":
        input_data["result"], input_data["mess"] = chat_re(input_data)
    elif task == "NER":
        input_data["result"], input_data["mess"] = chat_ner(input_data)
    else:
        input_data["result"], input_data["mess"] = chat_ee(input_data)
